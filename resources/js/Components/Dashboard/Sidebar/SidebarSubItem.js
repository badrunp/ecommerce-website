...
function SidebarSubItem({ data, index }) {
    ...
    return (
        <motion.div variants={itemSidebarVariants} custom={index} key={data.id} className="w-full">
            <Link href={data.link} className={`flex flex-row items-center justify-between space-x-4 py-1 w-full pl-6 ${url.startsWith(`/dashboard/products/${data.title.toLowerCase()}`) ? 'bg-gray-50' : ''}`}>
